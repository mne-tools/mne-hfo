...
def generate_cli_rst(app=None):
    ...
    cli_path = op.abspath(
        op.join(os.path.dirname(__file__), '..', '..', 'mne_hfo', 'commands'))
    fnames = sorted([
        op.basename(fname)
        for fname in glob.glob(op.join(cli_path, 'mne_hfo*.py'))])
    ...
    with open(out_fname, 'w', encoding='utf-8') as f:
        ...
        for fname in iterator:
            ...
            for li, line in enumerate(output):
                if line.startswith('Usage: mne_hfo '):
            ...
            f.write(command_rst % (cmd_name,
                                   cmd_name.replace('mne_bids_', 'mne_hfo '),
                                   '=' * len(cmd_name),
                                   output))
