...
def test_find_coincident_events():
    df1 = {"01": [(0.0, 6.73), (12.6, 14.87), (22.342, 31.1), (45.9, 67.2)]}
    df2 = {"01": [(0.2, 6.93), (12.3, 15.12), (45.8, 65.6), (98.3, 101.45)]}
    # Expect to return the first three events from df1,
    # which have overlap with df2
    expected_output = {"01": [(0.0, 6.73), (12.6, 14.87), (45.9, 67.2)]}
    coincident_df = find_coincident_events(df1, df2)
    assert expected_output == coincident_df

    df1["02"] = df1["01"]
    with pytest.raises(
        RuntimeError, match="The two dictionaries" " must have the same keys."
    ):
...
def test_match_hfo_annotations():
    ...
    expected_dict_true = {"true_index": [0, 1, 2, 3], "pred_index": [0, 1, None, 2]}
    expected_df_true = pd.DataFrame(expected_dict_true)
    expected_df_true = expected_df_true.apply(
        pd.to_numeric, errors="coerce", downcast="float"
    )
    output_df_true = match_detected_annotations(
        annot_df1, annot_df2, method="match-true"
    )
    pd.testing.assert_frame_equal(expected_df_true, output_df_true, check_dtype=False)

    # Now lets check what predicted labels correspond to true labels
    expected_dict_pred = {"pred_index": [0, 1, 2, 3], "true_index": [0, 1, 3, None]}
    ...
    pd.testing.assert_frame_equal(expected_df_pred, output_df_pred, check_dtype=False)
    ...
    expected_df_total = expected_df_total.apply(
        pd.to_numeric, errors="coerce", downcast="float"
    )
    output_df_total = match_detected_annotations(
        annot_df1, annot_df2, method="match-total"
    )
    pd.testing.assert_frame_equal(expected_df_total, output_df_total, check_dtype=False)

    # Error should be thrown for any other passed methods
    with pytest.raises(NotImplementedError, match=""):
        match_detected_annotations(annot_df1, annot_df2, method="match-average")
...
def test_metrics_df(end_sec, rate):
    ...
    chs_hfo_rates = compute_chs_hfo_rates(annot_df=annot_df, rate=rate, end_sec=end_sec)
    assert chs_hfo_rates["A2"] == chs_hfo_rates["A3"]
    np.testing.assert_almost_equal(
        chs_hfo_rates["A2"], (1.0 / end_sec) / TIME_SCALE_TO_SECS[rate], decimal=6
    )
    np.testing.assert_almost_equal(
        chs_hfo_rates["A1"], (2.0 / end_sec) / TIME_SCALE_TO_SECS[rate], decimal=6
    )
